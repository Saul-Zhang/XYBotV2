...
from database.XYBotDB import User
...
class GetContact(PluginBase):
    ...
    async def fetch_and_save_contacts(bot):
        ...
        for contact in clean_info:
            user = User(wxid=contact["Wxid"], nickname=contact["Nickname"], remark=contact["Remark"], wx_num=contact["Alias"], big_head_img_url=contact["BigHeadImgUrl"])
